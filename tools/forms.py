...
class ToolForm(forms.ModelForm):
    ...
    has_been_maintained = forms.BooleanField(label='¿Ha tenido mantenimiento?', required=False)
    mainteance_date = forms.DateField(
        label='Fecha de mantenimiento',
        widget=forms.SelectDateWidget(years=range(2010, 2021))
    )
    ...
    class Meta:
        model = Tool
        fields = ('name', 'serial_number', 'model', 'has_been_maintained',
